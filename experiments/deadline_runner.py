...
METHODS: dict[str, Callable[..., RunResult]] = {
    "flat": run_flat,
    "flat_parallel": run_flat_parallel,
...
def run_job(job: Job) -> RunRow:
    ...
    if job.method in {"flat_parallel", "factorized", "kernel_memo_parallel"}:
...
def summarize(
    ...
    *,
    ...
    for case in sorted({row.case for row in method_rows}):
        ...
        flat_parallel_to_memo_parallel: list[float] = []
        flat_parallel_to_factorized: list[float] = []
        for group in by_repeat.values():
            ...
            flat_parallel_rows = [
                row for row in group if row.method == "flat_parallel"
            ]
            ...
            for flat_parallel in flat_parallel_rows:
                memo_parallel = lookup.get(
                    ("kernel_memo_parallel", flat_parallel.workers)
                )
                factor_parallel = lookup.get(
                    ("factorized", flat_parallel.workers)
                )
                if memo_parallel is not None:
                    flat_parallel_to_memo_parallel.append(
                        flat_parallel.elapsed_s / memo_parallel.elapsed_s
                    )
                if factor_parallel is not None:
                    flat_parallel_to_factorized.append(
                        flat_parallel.elapsed_s / factor_parallel.elapsed_s
                    )
        ...
        method_effects.append(
            {
                ...
                "flat_parallel_pair_repeats": len(
                    flat_parallel_to_factorized
                ),
                ...
                "parallel_flat_to_kernel_memo_paired_median": (
                    statistics.median(flat_parallel_to_memo_parallel)
                    if flat_parallel_to_memo_parallel
                    else None
                ),
                "parallel_flat_to_factorized_paired_median": (
                    statistics.median(flat_parallel_to_factorized)
                    if flat_parallel_to_factorized
                    else None
                ),
    ...
    completed_job_ids = {row.job_id for row in rows}
    selected_complete = (
        completed_job_ids == planned_job_ids
        and selected_studies <= completed_studies
        and len(rows) == len(planned_job_ids)
    )
    return {
        "status": (
            (
                "paper_candidate_results_semantics_checked"
                if selected_studies == all_studies
                else "selected_results_semantics_checked"
            )
            if selected_complete
            else "partial_results_semantics_checked"
        ),
        "selected_studies": sorted(selected_studies),
...
def write_summary_markdown(summary: dict[str, object], path: Path) -> None:
    ...
    for item in summary["method_effects"]:
        matched_flat = item["parallel_flat_to_factorized_paired_median"]
        lines.append(
            ...
            f"{'NA' if matched_flat is None else f'{matched_flat:.3f}×'} | "
...
def main() -> None:
    ...
    summary = summarize(
        rows,
        metadata,
        selected_studies={job.study for job in jobs},
